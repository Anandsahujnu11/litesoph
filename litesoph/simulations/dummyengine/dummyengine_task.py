...
from litesoph.simulations.dummyengine.dummyengine import dummyengine
from litesoph.simulations.dummyengine.spectrum import photoabsorption_spectrum
...
class dummyengineTask(Task):
    ...
    def create_engine(self, param):
        ...
        infile_ext = '.nwi'
        outfile_ext = '.nwo'
        self.task_dir = self.project_dir / 'dummyengine' / self.task_data.get('dir')
        ...
        if self.task_name in self.post_processing_tasks:
            ...
            self.nwchem = dummyengine(outfile=str(self.outfile), 
        ...
        if 'rt_tddft' in self.task_name:
            ...
            param['basis'] = self.status.get_status('dummyengine.ground_state.param').get('basis')
        
        file_name = self.task_data.get('file_name')
        self.infile = file_name + infile_ext
        self.outfile = file_name + outfile_ext
        self.nwchem = dummyengine(infile= self.infile, outfile=self.outfile, 
    ...
    def create_template(self):
        self.template = self.dummyengine.create_input()
    ...
    def _create_spectrum_cmd(self, remote=False ):
        ...
        nw_rtparse = str(path.parent /'dummyengine_read_rt.py')
    ...
    def create_job_script(self, np=1, remote_path=None) -> list:
        ...
        path_dummyengine = self.lsconfig.get('engine', 'dummyengine')

        if not path_dummyengine or remote_path:
            path_dummyengine = 'dummyengine'

        if self.task_name in self.simulation_tasks:
            ifilename =  self.infile
            engine_cmd = path_dummyengine + ' ' + str(ifilename) + ' ' + '>' + ' ' + str(ofilename)
...
def get_pol_and_tag(status):
    ...
    param =  status.get_status('dummyengine.rt_tddft_delta.param')
